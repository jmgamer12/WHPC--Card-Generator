...
class BirthdayHandler(webapp2.RequestHandler):
    def get(self):
        ...
        if bdName:
            ...
            self.response.write('''<html><body><center><br><p> To Who? </p><br><img src="/images/owl.jpg" /> <br><br><br><br><br><br> <a href="/"> Home </a></center><br></body></html>''')
    ...
    def get(self):
        ...
        if xmasName:
            ...
            if not gif_url:
                ...
                x = len(parsed_giphy_dictionary['data'])
                gif_url = parsed_giphy_dictionary['data'][random.randint(0, x-1)]['images']['original']['url']
                self.redirect('/templates/xmas-card?xmasName=%s&xmasYName=%s&gif=%s' % (xmasName, xmasYName, gif_url))
            textRand = random.randint(1, 2)
            if textRand == 1:
